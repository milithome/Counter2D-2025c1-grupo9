...
ServerConfig &Admin::getServerConfig() { return serverConfig; }
...
std::shared_ptr<Client> Admin::createClient(Protocol &&protocol) {
  ...
  client->channels = ClientChannels{"", std::make_shared<Queue<Message>>(100),
                                    std::make_shared<Queue<Response>>(100)};

  unnamedClients.insert(client);

  client->receiver = std::make_shared<ClientReceiver>(
      client->protocol, client->channels.name, client->channels.requests,
      [this, client](const std::string &name) {
        ...
        }
        {
          ...
          auto nameExists =
              std::any_of(this->clients.begin(), this->clients.end(),
        ...
        client->sender = std::make_shared<ClientSender>(
            client->protocol, client->channels.name,
...
void Admin::joinMatch(const std::string &matchName,
