...
TEST(ProtocolClientSender, SendAndReceiveLeaveLobbyMessage) {
...
TEST(ProtocolClientSender, SendAndReceiveDisconnectMessage) {
    Socket listening_socket("12352");
    ...
    std::thread server_thread([&]() {
        ...
        Message msg = server_protocol.recv_message();
        EXPECT_EQ(msg.type, Type::DISCONNECT);
    });

    wait_short_time();
    Socket client_socket("localhost", "12352");
    Protocol client_protocol(std::move(client_socket));
