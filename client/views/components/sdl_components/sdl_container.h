...
public:
    ...
    void build() {
        switch (child.getSizePolicy()) {
            case FIXED: {
                child.setH(std::min(rect.GetH() - margin*2, child.getH()));
                child.setW(std::min(rect.GetW() - margin*2, child.getW()));
            }
            case EXPAND: {
                child.setH(rect.GetH() - margin*2);
                child.setW(rect.GetW() - margin*2);
            }
            case EXPAND_BUT_MAINTAIN_RATIO: {

                uint32_t max_w = rect.GetW() - margin * 2;
                uint32_t max_h = rect.GetH() - margin * 2;
        ...
        switch (alignment.vertical) {
            case BOTTOM: {
                child.setY(rect.GetY() + rect.GetH() - child.getH() - margin);
                break;
            }
            case TOP: {
                child.setY(rect.GetY() + margin);
                break;
            }
            case MIDDLE: {
        ...
        switch (alignment.horizontal) {
            ...
            case RIGHT: {
                child.setX(rect.GetX() + rect.GetW() - child.getW() - margin);
                break;
            }
        }
    }
